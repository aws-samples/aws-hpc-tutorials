...
```bash
aws cloudformation deploy --stack-name nextflow-batch-jd --template-file sca23-nextflow-batch-jd.template.yaml --capabilities CAPABILITY_IAM --region ${AWS_REGION} --parameter-overrides  NFJobQueue=${NFJobQueue} BucketNameResults=${BucketNameResults} ImageId=${ImageId} ECSRoleArn=${ECSRoleArn}
```
...
![AWS Batch](/images/aws-batch/SCA23/jd-7.png)
